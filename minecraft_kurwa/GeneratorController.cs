        ...
        internal static void GenerateWorld() {
            ...
            for (ushort x = 0; x < Settings.WORLD_SIZE; x++) {
                for (ushort y = 0; y < Settings.WORLD_SIZE; y++) {
                    byte[] topBlocks = Biome.GetTopBlocks(Global.BIOME_MAP[x, y]);
                    Global.VOXEL_MAP[x, y, Global.HEIGHT_MAP[x, y]] = topBlocks[random.Next(0, topBlocks.Length - 1)];
                    if (Global.BIOME_MAP[x, y] == (byte) BiomeType.SUBPOLAR) Console.WriteLine("Subpolar");
