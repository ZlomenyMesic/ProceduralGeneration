        ...
        internal static byte[,] GenerateBiomeMap() {
            ...
            for (int x = 0; x < Global.WORLD_SIZE; x++) {
                for (int y = 0; y < Global.WORLD_SIZE; y++) {
                    ...
                    if ((byte)BiomeType.TEMPERATE_INLAND + Biome.TEMPERATE_INLAND_BIOME_COUNT < biomeType[x, y] && biomeType[x, y] < (int)BiomeType.COLD) biomeType[x, y] = 41;
                    if ((byte)BiomeType.COLD + Biome.COLD_BIOME_COUNT < biomeType[x, y]) biomeType[x, y] = 51;
                }
            }
