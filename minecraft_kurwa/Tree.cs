...
namespace minecraft_kurwa {
    internal static class Tree {
        internal static void GenerateTrees() {
            ushort amount = Global.WORLD_SIZE * Global.WORLD_SIZE * Global.TREE_DENSITY / 10_000;
            Random random = new(Global.SEED + 69);

            for (ushort i = 0; i < amount; i++) {
                ushort x = (ushort)random.Next(Global.TREE_EDGE_OFFSET, Global.WORLD_SIZE - Global.TREE_EDGE_OFFSET - 1);
                ushort z = (ushort)random.Next(Global.TREE_EDGE_OFFSET, Global.WORLD_SIZE - Global.TREE_EDGE_OFFSET - 1);
