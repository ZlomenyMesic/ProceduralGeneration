...
namespace minecraft_kurwa {
    internal static class Tree {
        internal static void GenerateTrees() {
            ushort maxCount = Settings.WORLD_SIZE * Settings.WORLD_SIZE * Settings.TREE_DENSITY / 10_000;
            ...
            for (ushort i = 0; i < maxCount; i++) {
                ...
                if (biome == 32 || biome == 42) {
                    switch (random.Next(0, 50)) {
                        case 0: BuildBasicDeciduousTree(random.Next(15, 19), x, y, Global.HEIGHT_MAP[x, y] + 1, VoxelType.CHERRY_LEAVES, VoxelType.CHERRY_WOOD); break;
                        case 1 or 2 or 3: BuildSpruceTree(random.Next(16, 22), x, y, Global.HEIGHT_MAP[x, y] + 1); break;
                        case 4 or 5 or 6 or 7: BuildBasicDeciduousTree(random.Next(13, 18), x, y, Global.HEIGHT_MAP[x, y] + 1, VoxelType.BEECH_LEAVES, VoxelType.BEECH_WOOD); break;
                        case 8 or 9 or 10 or 11: BuildPoplarTree(random.Next(22, 25), x, y, Global.HEIGHT_MAP[x, y] + 1); break;
                        case 12 or 13 or 14 or 15 or 16 or 17: BuildBasicDeciduousTree(random.Next(13, 16), x, y, Global.HEIGHT_MAP[x, y] + 1, VoxelType.MAPLE_LEAVES, VoxelType.MAPLE_WOOD); break;
                        default: BuildBasicDeciduousTree(random.Next(10, 18), x, y, Global.HEIGHT_MAP[x, y] + 1, VoxelType.OAK_LEAVES, VoxelType.OAK_WOOD); break;
                    }
                }
                else if (biome == 11) {
                    BuildKapokTree(random.Next(15, 18), x, y, Global.HEIGHT_MAP[x, y] + 1);
                }
                else if (biome == 31 || biome == 41 || biome == 51 || biome == 61 || biome == 63) {
                    BuildSpruceTree(random.Next(16, 22), x, y, Global.HEIGHT_MAP[x, y] + 1);
                }
            }
        }
